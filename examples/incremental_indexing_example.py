...
try:
    ...
    logger.error("Error importing Agent-S3 modules: %s", e)
...
def main():
    ...
    if not os.path.isdir(args.repo_path):
        logger.error(
            "Repository path does not exist or is not a directory: %s",
            args.repo_path,
        )
        sys.exit(1)
    ...
    try:
        ...
        logger.info(
            f"{'Full' if args.full else 'Incremental'} indexing of repository: {args.repo_path}"
        )
        ...
        if 'partitions' in stats:
            ...
            logger.info(
                "Total partitions: %s",
                partition_stats.get('total_partitions', 0),
            )
            logger.info(
                "Total files in index: %s",
                partition_stats.get('total_files', 0),
            )

        # Enable watch mode if requested
        if args.watch:
            logger.info("Enabling watch mode for repository: %s", args.repo_path)
            watch_id = adapter.enable_watch_mode(args.repo_path)
            if watch_id:
                logger.info("Watch mode enabled with ID: %s", watch_id)
        ...
        if args.search:
            logger.info("Searching for: %s", args.search)
            results = code_analysis_tool.search_code(args.search, top_k=5)

            # Display results
            logger.info("Found %s results:", len(results))
            for i, result in enumerate(results):
                logger.info("Result %s:", i + 1)
                logger.info("  File: %s", result.get('file', 'Unknown'))
                logger.info("  Score: %.4f", result.get('score', 0))
