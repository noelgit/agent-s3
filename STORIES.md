...
2.  **VS Code Extension Action (`vscode/extension.ts` - for UI triggers):**
    ...
    *   It uses `HttpClient` to send the command. If the HTTP call fails, the extension falls back to CLI execution.
...
4.  **Coordinator Workflow (`agent_s3/coordinator.py` - `run_task` method):**
    ...
    *   **Feature Group Processing:**
        ...
        *   **GitHub Integration:** When user accepts the plan ("yes") and a GitHub token is available, Agent-S3 automatically creates a GitHub issue containing the full plan details, test specifications, and implementation steps.
    ...
    *   **Finalization:**
        *   If changes were successfully made and validated, calls `_finalize_task(changes)` which:
            ...
            *   **GitHub Integration:** Automatically creates a GitHub pull request that references the original issue (if created) and includes implementation summary, file changes, and validation results
...
5.  **VS Code Monitoring (`vscode/extension.ts`):**
    *   The extension reads command output directly from the terminal; it does not poll `/status` or use a file watcher.
    ...
    *   **GitHub Integration:** Notifications include links to automatically created GitHub issues and pull requests when available.
...
10. **Chat UI Feedback:**
    ...
    *   `ChatView` displays the final result; streaming handlers exist but are not used in this version.
