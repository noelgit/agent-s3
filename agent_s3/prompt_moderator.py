...
Focus on explaining the "why" behind the decisions, not just describing what's in the JSON."""
    ...
    def present_consolidated_plan(self, plan: Dict[str, Any]) -> Tuple[str, Optional[str]]:
        ...
        if decision == "modify":
            ...
            # Notify about GitHub integration if enabled
            self._notify_github_integration_if_enabled(plan)
    ...
    def _notify_github_integration_if_enabled(self, plan: Dict[str, Any]) -> None:
        """Notify user about GitHub integration if GitTool has a token."""
        try:
