            ...
            for method_name in ["shutdown", "close", "cleanup"]:
                if hasattr(tool, method_name) and callable(getattr(tool, method_name)):
                    try:
                        ...
                        logger.debug(f"Successfully shutdown tool: {tool_name}")
...
class ContextSizeMonitor:
    ...
    - Maintains history of context growth
    - Provides alerts when thresholds are approached
    - Offers size estimation for planning
    """
    
    def __init__(self, token_analyzer, max_tokens: int = 16000):
        """
        Initialize the context size monitor.
        
        Args:
            token_analyzer: Tool to count tokens in text
            max_tokens: Maximum token limit to monitor against
        """
        self.token_analyzer = token_analyzer
        self.max_tokens = max_tokens
        self.current_usage = 0
        self.section_usage: Dict[str, int] = {}
        self.history: List[Tuple[float, int]] = []  # [(timestamp, token_count)]
        self.alert_thresholds = [0.7, 0.8, 0.9]  # 70%, 80%, 90% of max
        self._exceeded_thresholds: Set[float] = set()
    
    def update(self, context: Dict[str, Any]) -> None:
        """
        Update token usage statistics based on the current context.
        ...
        # Keep history to a reasonable size by pruning old entries
        if len(self.history) > 100:
            self.history = self.history[-100:]
        
        # Update section-specific usage
    ...
    def _check_thresholds(self) -> None:
        ...
        for threshold in self.alert_thresholds:
            if usage_ratio >= threshold and threshold not in self._exceeded_thresholds:
                logger.warning(f"Context size alert: {int(threshold * 100)}% of token budget used "
                ...
                # Reset threshold if we go back below it
                self._exceeded_thresholds.remove(threshold)
    
    def get_section_breakdown(self) -> Dict[str, Dict[str, Any]]:
        """
        Get detailed breakdown of token usage by section.
        
        Returns:
            Dictionary with section usage stats
        """
        total = max(1, self.current_usage)  # Avoid division by zero
        ...
        for section, tokens in self.section_usage.items():
            result[section] = {
                ...
                "is_large": tokens > (self.max_tokens * 0.2)  # Flag sections using >20% of budget
            }
    ...
    def get_growth_rate(self) -> float:
        """
        Calculate the token usage growth rate over time.
        ...
        # Calculate growth over last 5 points or all if fewer
    ...
    def estimate_time_to_threshold(self, threshold_ratio: float = 0.9) -> Optional[float]:
        """
        Estimate time until a specific threshold is reached.
        
        Args:
            threshold_ratio: Threshold as ratio of max_tokens (0.0-1.0)
            
        Returns:
            Estimated seconds until threshold or None if growth rate is zero/negative
        """
...
class ContentPruningManager:
    ...
    Features:
    - Tracks access patterns for context elements
    - Assigns value scores based on recency, frequency, and semantic importance
    - Provides prioritized pruning recommendations when token budgets are exceeded
    - Preserves critical context components during pruning operations
    """
    
    def __init__(self, token_analyzer):
        """
        Initialize the content pruning manager.
        ...
        self._importance_overrides: Dict[str, float] = {}  # Manual importance scoring
        self._critical_paths: Set[str] = set()  # Paths that should never be pruned
        self._lru_cache = OrderedDict()  # For efficient LRU tracking
        
        # How many recent accesses to track per key
    ...
    def record_access(self, key_path: str) -> None:
        """
        Record an access to a context element to track LRU.
        
        Args:
            key_path: The dot-notation path to the element that was accessed
        """
        timestamp = time.time()
        ...
        if len(history) > self._max_history_per_key:
            history.pop(0)  # Keep history bounded
        ...
        # Keep LRU cache to a reasonable size
        if len(self._lru_cache) > 1000:
            self._lru_cache.popitem(last=False)  # Remove oldest element
    
    def set_importance(self, key_path: str, importance: float) -> None:
        """
        Set a manual importance override for a specific context element.
        
        Args:
            key_path: The dot-notation path to the element
    ...
    def mark_as_critical(self, key_path: str) -> None:
        """
        Mark a context element as critical (should never be pruned).
    ...
    def _calculate_value_score(self, key_path: str) -> float:
        """
        Calculate the overall value score for a context element.
        
        Args:
            key_path: The dot-notation path to the element
        
        Returns:
            Value score (0.0-1.0) where higher means more valuable
        """
        # If it's critical, always return maximum value
        if key_path in self._critical_paths:
            return 1.0
            
        # Use importance override if available
        if key_path in self._importance_overrides:
            return self._importance_overrides[key_path]
            
        # Calculate recency score based on most recent access
        recency_score = 0.0
        ...
        # Calculate frequency score based on normalized access count
        freq_score = 0.0
        ...
        # Calculate importance based on other factors
        # Default to middle importance if no other data
    ...
    def identify_pruning_candidates(self, context: Dict[str, Any], 
                                   current_tokens: int, 
                                   target_tokens: int) -> List[Tuple[str, float, int]]:
        """
        Identify elements to prune when the context exceeds token budget.
        ...
        Returns:
            List of (key_path, value_score, token_count) tuples sorted by value (lowest first)
        """
        ...
        # Collect all pruneable elements and their stats
    ...
    def _collect_pruneable_elements(self, context: Dict[str, Any], 
                                  prefix: str, 
                                  result: List[Tuple[str, int]]) -> None:
        """
        Recursively collect pruneable elements from the context.
        
        Args:
            context: Context dictionary or subdictionary
            prefix: Current key path prefix
            result: Output list to append (key_path, token_count) tuples to
        """
        ...
        for key, value in context.items():
            ...
            if isinstance(value, str):
                ...
                # Option 1: Include the whole dict as a candidate
                total_tokens = self.token_analyzer.get_token_count(json.dumps(value))
                if total_tokens > 0:
                    result.append((current_path, total_tokens))
                    
                # Option 2: Also include individual elements for more granular pruning
...
class ContextManager:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        if self.config.get("adaptive_config", {}).get("enabled", False):
            try:
                ...
                logger.info(f"Initializing adaptive configuration for repository: {repo_path}")
                ...
                if adaptive_settings and "context_management" in adaptive_settings:
                    logger.info(f"Using adaptive configuration (version {self.adaptive_config_manager.get_config_version()})")
    ...
    def _update_nested_dict(self, target_dict: Dict[str, Any], key_path: Union[str, List[str]], new_value: Any) -> None:
        """
    ...
    def initialize_tools(self, tech_stack_detector=None, code_analysis_tool=None, 
        ...
        logger.info(f"Context Manager initialized with {len(self._tool_registry._tools)} tools")
        ...
        try:
            ...
            if importance_scores:  # Ensure the map is not empty
                for section_key, section_value in importance_scores.items():
                    if isinstance(section_value, dict):  # Handles nested structures like 'code_context'
                        ...
                        logger.warning(f"Unexpected structure in importance_scores for section '{section_key}'. Type: {type(section_value)}. Skipping propagation for this section.")
            ...
            if current_tokens > target_tokens:
                ...
                for key_path, value_score, token_count in pruning_candidates:
                    ...
                    logger.debug(f"Pruned {token_count} tokens from {key_path} (value score: {value_score:.2f})")
            compressed_context = {}
    ...
    def _refine_current_context(self, files: List[str], max_tokens: int = None, task_keywords: Optional[List[str]] = None) -> None:
        """
