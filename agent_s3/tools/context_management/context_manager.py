...
class ContextManager:
    ...
    def initialize_tools(self, tech_stack_detector=None, code_analysis_tool=None,
        ...
        logger.info("Context Manager initialized with %s tools", len(self._tool_registry._tools))
    ...
    def _optimize_context(self) -> None:
        ...
        try:
            ...
            if importance_scores:  # Ensure the map is not empty
                for section_key, section_value in importance_scores.items():
                    if isinstance(section_value, dict):  # Handles nested structures like 'code_context'
                        ...
                        logger.warning(
                            "Unexpected structure in importance_scores for section '%s'. Type: %s. Skipping propagation for this section.",
                            section_key,
                            type(section_value),
                        )
            ...
            logger.error("Error during context optimization: %s", e)
        ...
        for file_path in prioritized_files:
            ...
            try:
                ...
                if content_tokens > (max_tokens * 0.4):  # Skip if file would use >40% of budget
                    logger.info("Skipping large file %s (%s tokens)", file_path, content_tokens)
                    continue
    ...
    def set_allocation_strategy(self, strategy: 'DynamicAllocationStrategy') -> None:
        ...
        logger.info("Setting new allocation strategy: %s", strategy.__class__.__name__)
        ...
        if self.adaptive_config_manager:
            ...
            if adaptive_settings and "context_management" in adaptive_settings:
                ...
                if embedding_config:
                    ...
                    if chunk_overlap:
                        logger.debug("Using adaptive chunk overlap: %s", chunk_overlap)
                    ...
                    if embedding_model:
                        logger.debug("Using adaptive embedding model: %s", embedding_model)
                ...
                if bm25_config:
                    ...
                    b = bm25_config.get("b")
                    if k1:
                        logger.debug("Using adaptive BM25 k1: %s", k1)
                        self.config.setdefault("search_params", {}).setdefault("bm25", {})["k1"] = k1
                    if b:
                        logger.debug("Using adaptive BM25 b: %s", b)
                ...
                if vector_config:
                    ...
                    if top_k:
                        logger.debug("Using adaptive vector search top_k: %s", top_k)
                        self.config.setdefault("search_params", {}).setdefault("vector", {})["top_k"] = top_k

                    if similarity_threshold:
                        logger.debug("Using adaptive similarity threshold: %s", similarity_threshold)
                        self.config.setdefault("search_params", {}).setdefault("vector", {})["similarity_threshold"] = similarity_threshold

                    if similarity_metric:
                        logger.debug("Using adaptive similarity metric: %s", similarity_metric)
                ...
                if hybrid_config:
                    ...
                    if bm25_weight:
                        logger.debug("Using adaptive hybrid search BM25 weight: %s", bm25_weight)
                        self.config.setdefault("search_params", {}).setdefault("hybrid", {})["bm25_weight"] = bm25_weight

                    if vector_weight:
                        logger.debug("Using adaptive hybrid search vector weight: %s", vector_weight)
                ...
                if summarization_config:
                    ...
                    if threshold:
                        logger.debug("Using adaptive summarization threshold: %s", threshold)
                    ...
                    if compression_ratio:
                        logger.debug("Using adaptive compression ratio: %s", compression_ratio)
                    ...
                    if min_tokens:
                        logger.debug("Using adaptive min tokens for summarization: %s", min_tokens)
                        self.config.setdefault("summarization", {})["min_tokens"] = min_tokens

                    if max_tokens:
                        logger.debug("Using adaptive max tokens for summarization: %s", max_tokens)
                ...
                if importance_scoring and isinstance(self.allocation_strategy, TaskAdaptiveAllocation):
                    ...
                    if code_weight:
                        logger.debug("Using adaptive code weight: %s", code_weight)
                        self.allocation_strategy.code_weight = code_weight

                    if comment_weight:
                        logger.debug("Using adaptive comment weight: %s", comment_weight)
                        self.allocation_strategy.comment_weight = comment_weight

                    if metadata_weight:
                        logger.debug("Using adaptive metadata weight: %s", metadata_weight)
                        self.allocation_strategy.metadata_weight = metadata_weight

                    if framework_weight:
                        logger.debug("Using adaptive framework weight: %s", framework_weight)
                    ...
                    if test_weight and hasattr(self.allocation_strategy, 'test_weight'):
                        logger.debug("Using adaptive test weight: %s", test_weight)
                    ...
                    if documentation_weight and hasattr(self.allocation_strategy, 'documentation_weight'):
                        logger.debug("Using adaptive documentation weight: %s", documentation_weight)
                ...
                if token_budget:
                    ...
                    if max_context_tokens:
                        logger.debug("Using adaptive max context tokens: %s", max_context_tokens)
                    ...
                    if reserved_tokens:
                        logger.debug("Using adaptive reserved tokens: %s", reserved_tokens)
                ...
                if background_opt:
                    ...
                    if enabled is not None:
                        logger.debug("Setting background optimization enabled: %s", enabled)
                    ...
                    if interval:
                        logger.debug("Using adaptive optimization interval: %s", interval)
                ...
                if pruning_config and hasattr(self, '_pruning_manager'):
                    ...
                    if recency_weight:
                        logger.debug("Using adaptive pruning recency weight: %s", recency_weight)
                        self._pruning_manager.recency_weight = recency_weight

                    if frequency_weight:
                        logger.debug("Using adaptive pruning frequency weight: %s", frequency_weight)
                        self._pruning_manager.frequency_weight = frequency_weight

                    if importance_weight:
                        logger.debug("Using adaptive pruning importance weight: %s", importance_weight)
                        self._pruning_manager.importance_weight = importance_weight

                logger.info(
                    "Successfully applied adaptive configuration (version %s)",
                ...
                if hasattr(self.adaptive_config_manager.metrics_collector, 'register_callback'):
                    try:
                        ...
                        logger.warning("Failed to register metrics callback: %s", e)
    ...
    def get_detected_frameworks(self) -> List[str]:
        ...
        try:
            ...
            logger.error("Error getting frameworks: %s", e)
            return []
    ...
    def get_tech_stack_details(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error("Error getting tech stack details: %s", e)
            return {}
    ...
    def get_package_dependencies(self) -> Dict[str, str]:
        ...
        try:
            ...
            logger.error("Error getting package dependencies: %s", e)
            return {}
    ...
    def get_relevant_files(self, query: str) -> List[str]:
        ...
        try:
            ...
            logger.error("Error searching for relevant files: %s", e)
            return []
    ...
    def get_current_working_directory(self) -> str:
        ...
        try:
            ...
            logger.error("Error getting current working directory: %s", e)
            return ""
    ...
    def get_test_frameworks(self) -> List[str]:
        ...
        try:
            ...
            logger.error("Error getting test frameworks: %s", e)
            return []
    ...
    def get_test_files(self) -> List[str]:
        ...
        try:
            ...
            logger.error("Error getting test files: %s", e)
            return []
    ...
    def get_dependency_graph(self) -> Dict[str, Any]:
        ...
        if code_tool and hasattr(code_tool, "get_dependency_graph"):
            try:
                ...
                logger.error("Error refreshing dependency graph: %s", e)
    ...
    def _log_metrics_to_adaptive_config(self, context_used: Dict[str, Any],
        ...
        if not isinstance(context_used, dict):
            logger.error(
                "Context data is missing or malformed when logging metrics",
            )
            return
        ...
        try:
            ...
            logger.error("Error logging metrics to adaptive config: %s", e)
    ...
    def shutdown(self) -> None:
        ...
        if self.adaptive_config_manager:
            ...
            try:
                ...
                logger.error("Error during final optimization: %s", e)
