...
from agent_s3.tools.context_management.adaptive_config import (
    AdaptiveConfigManager, ConfigExplainer, ProjectProfiler
)
...
class CoordinatorContextIntegration:
    ...
    def __init__(self, coordinator: Any, context_manager: Optional[ContextManager] = None):
        ...
        if context_manager is None:
            ...
            if config.get('context_management', {}).get('adaptive_config', {}).get('enabled', True):
                ...
                if self.adaptive_config_manager:
                    ...
                    if adaptive_config and 'context_management' in adaptive_config:
                        ...
                        config['context_management'].update(adaptive_config['context_management'])
            ...
            if self.adaptive_config_manager:
                self.context_manager.set_adaptive_config_manager(self.adaptive_config_manager)
        else:
    ...
    def _get_repository_path(self) -> Optional[str]:
        ...
        elif hasattr(self.coordinator, 'config') and hasattr(self.coordinator.config, 'project_path'):
    ...
    def _setup_adaptive_configuration(self, config: Dict[str, Any]) -> None:
        ...
        if not self.repo_path:
            logger.warning("Cannot set up adaptive configuration without repository path")
            return
            
        try:
            # Determine configuration directory
            config_dir = config.get('context_management', {}).get(
                'adaptive_config', {}).get('config_dir')
            ...
            logger.info(f"Initialized adaptive configuration system in {config_dir}")
        except Exception as e:
            logger.error(f"Failed to set up adaptive configuration: {e}")
    ...
    def integrate(self) -> bool:
        ...
        try:
            ...
            if self.adaptive_config_manager:
                ...
                setattr(self.coordinator, 'get_config_explanation', get_config_explanation)
            ...
            logger.error(f"Failed to integrate context management with Coordinator: {e}")
            logger.debug(traceback.format_exc())
            return False
    ...
    def _integrate_file_tracking(self) -> None:
        ...
        original_get_file_modification_info = getattr(self.coordinator, 'get_file_modification_info', None)
        
        if original_get_file_modification_info is None:
            logger.warning("Couldn't find get_file_modification_info method on Coordinator")
            return
        ...
        # Apply patch
        setattr(self.coordinator, 'get_file_modification_info', patched_get_file_modification_info)
    ...
    def _integrate_router_agent(self) -> None:
        ...
        if original_route is not None:
            ...
            if original_call_llm is not None:
                def patched_call_llm_by_role(role, system_prompt, user_prompt, *args, **kwargs):
                    ...
                    if code_context:
                        ...
                        optimized = self.context_manager.optimize_context(context, model_name)
                    ...
                    return original_call_llm(role, system_prompt, user_prompt, *args, **kwargs)
        ...
        def patched_shutdown():
            ...
            if hasattr(self.coordinator, 'context_manager') and self.coordinator.context_manager:
                ...
                if hasattr(self.coordinator.context_manager, 'adaptive_config_manager') and \
                   self.coordinator.context_manager.adaptive_config_manager:
                    ...
                    try:
                        ...
                        logger.error(f"Error finalizing adaptive configuration: {e}")
    ...
    def _integrate_config_monitoring(self) -> None:
        ...
        original_handle_completion = getattr(self.coordinator, 'handle_completion', None)
        ...
        if original_handle_completion:
            ...
            def patched_handle_completion(result, *args, **kwargs):
                ...
                try:
                    ...
                    logger.error(f"Error logging adaptive configuration metrics: {e}")
...
def setup_context_management(coordinator):
    ...
    try:
        ...
        from agent_s3.tools.context_management.llm_integration import integrate_with_llm_utils
        ...
        if hasattr(coordinator, 'tech_stack') and coordinator.tech_stack:
            context_manager.update_context({
                "tech_stack": coordinator.tech_stack
            })
            
            # If we have adaptive config, update the tech stack info there too
            if hasattr(coordinator, 'adaptive_config_manager') and coordinator.adaptive_config_manager:
        ...
        # Log error
        logger.error(f"Failed to set up context management: {e}")
        logger.debug(traceback.format_exc())
...
def get_configuration_report(coordinator):
    ...
    try:
        ...
        logger.error(f"Failed to get configuration report: {e}")
...
def optimize_configuration(coordinator, reason: str = "Manual optimization"):
    ...
    """
    if not hasattr(coordinator, 'adaptive_config_manager') or not coordinator.adaptive_config_manager:
...
def analyze_repository_profile(coordinator):
    ...
    try:
        ...
        logger.error(f"Failed to analyze repository profile: {e}")
        return {"error": str(e)}