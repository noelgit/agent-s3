...
class PhpPestAdapter(Adapter):
    ...
    def detect(self, workspace: Path) -> bool:
        ...
        if test_files:
                ...
                try:
                    with open(test_file, "r", encoding="utf-8") as f:
                        ...
                        if "test(" in content or "it(" in content:
                            logger.info(f"Found Pest-style tests in {test_file}")
                            self.is_pest = True
                            return True
                        elif "extends TestCase" in content or "PHPUnit" in content:
                            logger.info(f"Found PHPUnit-style tests in {test_file}")
                            self.is_phpunit = True
                            return True
                except Exception:
            ...
            logger.info(f"Found {len(test_files)} PHP test files, assuming PHPUnit")
            self.is_phpunit = True
            return True
    ...
    def collect_only(self, workspace: Path) -> List[str]:
        ...
        try:
            ...
            logger.error(f"Error collecting PHP tests: {str(e)}")
    ...
    def smoke_run(self, workspace: Path) -> bool:
        ...
        try:
            ...
            logger.error(f"Error running PHP smoke tests: {str(e)}")
            return False
    ...
    def coverage(self, workspace: Path) -> Optional[float]:
        ...
        try:
            ...
            logger.error(f"Error running PHP coverage analysis: {str(e)}")
            return None
    ...
    def mutation(self, workspace: Path) -> Optional[float]:
        ...
        try:
            ...
            logger.error(f"Error running PHP mutation testing: {str(e)}")
            return None
