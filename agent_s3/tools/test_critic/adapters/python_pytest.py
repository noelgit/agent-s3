...
class PythonPytestAdapter(Adapter):
    ...
    def detect(self, workspace: Path) -> bool:
        ...
        if test_files:
            logger.info(f"Found {len(test_files)} pytest-style test files")
            return True
            ...
            try:
                with open(py_file, "r", encoding="utf-8") as f:
                    ...
                    if "import pytest" in content or "from pytest" in content:
                        logger.info(f"Found pytest import in {py_file}")
                        return True
            except Exception:
    ...
    def collect_only(self, workspace: Path) -> List[str]:
        ...
        try:
            ...
            logger.error(f"Error running pytest collect-only: {str(e)}")
    ...
    def smoke_run(self, workspace: Path) -> bool:
        ...
        try:
            ...
            logger.error(f"Error running pytest smoke tests: {str(e)}")
            return False
    ...
    def mutation(self, workspace: Path) -> Optional[float]:
        ...
        try:
            ...
            if best_score < min_score:
                logger.warning(f"Mutation score {best_score:.1f}% below threshold of {min_score}%")
            ...
            logger.error(f"Error running mutation testing: {str(e)}")
            return None
