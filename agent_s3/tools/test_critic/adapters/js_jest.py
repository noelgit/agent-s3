...
class JsJestAdapter(Adapter):
    ...
    def detect(self, workspace: Path) -> bool:
        ...
        if test_files:
            logger.info(f"Found {len(test_files)} Jest-style test files")
            return True
    ...
    def collect_only(self, workspace: Path) -> List[str]:
        ...
        try:
            ...
            logger.error(f"Error listing Jest tests: {str(e)}")
    ...
    def smoke_run(self, workspace: Path) -> bool:
        ...
        try:
            ...
            logger.error(f"Error running Jest smoke tests: {str(e)}")
            return False
    ...
    def coverage(self, workspace: Path) -> Optional[float]:
        ...
        try:
            ...
            logger.error(f"Error running Jest coverage analysis: {str(e)}")
            return None
    ...
    def mutation(self, workspace: Path) -> Optional[float]:
        ...
        try:
            ...
            logger.error(f"Error running Stryker mutation testing: {str(e)}")
            return None
