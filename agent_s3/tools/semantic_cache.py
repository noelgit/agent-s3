...
class SemanticCache:
    ...
    def _init_embedding_client(self) -> None:
        ...
        try:
            ...
            logger.error(f"Failed to initialize embedding client: {e}. Semantic matching will be disabled.")
    ...
    def _init_vector_store(self) -> None:
        ...
        try:
            ...
            logger.error(f"Failed to initialize FAISS index: {e}. Semantic matching will be disabled.")
            self.index = None
                ...
                for key, entry in entries.items():
                    ...
                    if embedding_data and self.index is not None:
                        try:
                            ...
                            logger.warning(f"Failed to add embedding for cache key {key}: {e}")
                
                logger.info(f"Loaded {len(valid_entries)} valid cache entries from disk")
    ...
    def _save_cache(self) -> None:
        ...
        try:
            ...
            logger.debug(f"Saved {len(self.mem_cache)} cache entries to disk")
                    ...
                    if query_embedding is not None:
                        ...
                        if distances.size > 0 and distances[0][0] > self.similarity_threshold:
                            ...
                            if match_key and match_key in self.mem_cache:
                                ...
                                logger.info(f"Semantic cache hit with similarity {distances[0][0]:.3f} > threshold {self.similarity_threshold}")
                                return {'response': entry.get("response"), 'cached': True}
    ...
    def set(self, prompt_data: Dict[str, Any], response: Any) -> None:
        ...
        with self._lock:
            ...
            if self.embedding_client is not None and self.index is not None:
                try:
                    ...
                    if embedding is not None:
                        ...
                        logger.debug(f"Successfully generated embedding for cache entry: {key[:8]}...")
                except Exception as e:
            ...
            logger.debug(f"Added new cache entry: {key[:8]}...")
    ...
    def _evict_entries(self) -> None:
        ...
        logger.info(f"Evicting {num_to_evict} cache entries from semantic cache")
        ...
        if self.index is not None and index_positions_to_remove and hasattr(self.index, 'remove_ids'):
            try:
                ...
                logger.debug(f"Removed {len(remove_ids)} entries from FAISS index")
            except Exception as e:
                logger.warning(f"Error removing entries from FAISS index: {e}")
                ...
                if len(self.mem_cache) > 0:
                    try:
                        ...
                        logger.error(f"Failed to rebuild index after eviction: {rebuild_error}")
        ...
        for key, entry in self.mem_cache.items():
            if entry.get("embedding") is not None:
                try:
                    ...
                    logger.warning(f"Error adding entry to rebuilt index: {e}")
        
        logger.info(f"FAISS index rebuilt with {self.index.ntotal} entries")
