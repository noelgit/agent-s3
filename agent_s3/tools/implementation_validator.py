...
def repair_implementation_plan(
    ...
    if "incomplete_implementation" in issues_by_type or "missing_steps" in issues_by_type:
        repaired_plan = _repair_incomplete_implementations(
            repaired_plan,
            issues_by_type.get("incomplete_implementation", []) + issues_by_type.get("missing_steps", []),
            system_design,
...
def _calculate_implementation_metrics(
    ...
    """
    Calculate metrics for implementation plan quality assessment.
    
    Args:
        implementation_plan: The implementation plan
        element_ids: Set of element IDs from system design
        architecture_issues: List of architecture issues from architecture review
        test_requirements: Dictionary mapping element IDs to test requirements
        
    Returns:
        Dictionary of metrics
    """
    metrics = {
        "element_coverage_score": 0.0,
        "architecture_issue_addressal_score": 0.0,
        "test_coverage_score": 0.0,
        "implementation_completeness_score": 0.0,
        "overall_score": 0.0
    }
    ...
    element_coverage = len(implemented_element_ids) / max(len(element_ids), 1)
    metrics["element_coverage_score"] = element_coverage
    
    # Calculate architecture issue addressal
    critical_issues = [issue["id"] for issue in architecture_issues if issue["severity"].lower() in ["critical", "high"]]
    addressed_issues = set()
    ...
    addressed_critical_issues = [issue for issue in critical_issues if issue in addressed_issues]
    architecture_issue_addressal = len(addressed_critical_issues) / max(len(critical_issues), 1)
    metrics["architecture_issue_addressal_score"] = architecture_issue_addressal
    
    # Calculate test coverage
    elements_with_tests = set(test_requirements.keys())
    covered_test_elements = implemented_element_ids.intersection(elements_with_tests)
    test_coverage = len(covered_test_elements) / max(len(elements_with_tests), 1)
    metrics["test_coverage_score"] = test_coverage
        ...
        if isinstance(functions, list):
            for function in functions:
                if isinstance(function, dict):
                    ...
                    has_steps = "steps" in function and isinstance(function["steps"], list) and len(function["steps"]) > 0
                    has_edge_cases = "edge_cases" in function and isinstance(function["edge_cases"], list)
                    has_description = "description" in function and function["description"]
                    ...
                    completeness_scores.append(function_score)
    
    implementation_completeness = sum(completeness_scores) / max(len(completeness_scores), 1)
    metrics["implementation_completeness_score"] = implementation_completeness
    
    # Calculate overall score
    metrics["overall_score"] = (
        element_coverage * 0.25 +
        architecture_issue_addressal * 0.35 +
        test_coverage * 0.2 +
...
def _validate_implementation_quality(
    ...
    """
    Validate the quality of implementation plan.
    
    Args:
        implementation_plan: The implementation plan
        metrics: Dictionary of implementation metrics
        
    Returns:
        List of quality validation issues
    """
    issues = []
    ...
    if metrics.get("overall_score", 0.0) < 0.7:
        issues.append({
            "issue_type": "low_quality_score",
            "severity": "medium",
            "description": f"Implementation plan has a low overall quality score: {metrics.get('overall_score', 0.0):.2f}"
        })
    
    # Check element coverage
    if metrics.get("element_coverage_score", 0.0) < 0.9:
        issues.append({
            "issue_type": "incomplete_element_coverage",
            "severity": "high",
            "description": f"Implementation plan does not cover all elements from system design. Coverage: {metrics.get('element_coverage_score', 0.0):.2f}"
        })
    
    # Check implementation completeness
    if metrics.get("implementation_completeness_score", 0.0) < 0.8:
        issues.append({
            "issue_type": "incomplete_implementation_details",
            "severity": "medium",
            "description": f"Some implementations lack complete details. Completeness score: {metrics.get('implementation_completeness_score', 0.0):.2f}"
        })
    
    # Check for functions with minimal steps
    for file_path, functions in implementation_plan.items():
        if isinstance(functions, list):
            for function_idx, function in enumerate(functions):
                if isinstance(function, dict) and "steps" in function:
                    steps = function["steps"]
                    if isinstance(steps, list) and len(steps) < 2:
                        issues.append({
                            "issue_type": "insufficient_steps",
                            "severity": "low",
                            "description": f"Function at index {function_idx} in file '{file_path}' has insufficient implementation steps",
                            "file_path": file_path,
...
def _validate_implementation_security(
    ...
    """
    Validate the security aspects of implementation plan.
    ...
    # Identify security-related architecture issues
    security_issues = [
        issue for issue in architecture_issues 
        if issue["issue_type"] == "security_concern"
    ]
    
    # Check if security issues are addressed in the implementation plan
    for security_issue in security_issues:
        issue_id = security_issue["id"]
        
        # Check if the issue is addressed somewhere in the implementation plan
        addressed = False
        for file_path, functions in implementation_plan.items():
            if isinstance(functions, list):
                for function in functions:
                    if isinstance(function, dict) and "architecture_issues_addressed" in function:
                        arch_issues = function["architecture_issues_addressed"]
                        if isinstance(arch_issues, list) and issue_id in arch_issues:
                            addressed = True
                            
                            # Check if there are security-specific steps in the implementation
                            security_steps = False
                            steps = function.get("steps", [])
                            for step in steps:
                                if isinstance(step, dict):
                                    step_desc = step.get("step_description", "").lower()
                                    if any(keyword in step_desc for keyword in ["security", "sanitize", "validate", "authenticate", "authorize", "encrypt", "hash"]):
                                        security_steps = True
                                        break
                            
                            if not security_steps:
                                issues.append({
                                    "issue_type": "missing_security_implementation",
                                    "severity": "high",
                                    "description": f"Function addresses security issue '{issue_id}' but lacks specific security implementation steps",
                                    "file_path": file_path,
                                    "arch_issue_id": issue_id
                                })
        ...
        if not addressed:
            issues.append({
                ...
                "severity": "critical",
                "description": f"Security issue '{issue_id}' ({security_issue.get('description', '')}) is not addressed in the implementation plan",
                "arch_issue_id": issue_id
            })
    
    # Check for common security concerns in function implementations
    security_keywords = ["input", "password", "auth", "token", "credential", "user", "admin", "permission", "role", "access"]
    
    for file_path, functions in implementation_plan.items():
        if isinstance(functions, list):
            for function_idx, function in enumerate(functions):
                if isinstance(function, dict):
                    # Check if function might deal with security-sensitive operations
                    function_signature = function.get("function", "").lower()
                    function_description = function.get("description", "").lower()
                    
                    is_security_sensitive = any(keyword in function_signature for keyword in security_keywords) or any(keyword in function_description for keyword in security_keywords)
                    
                    if is_security_sensitive:
                        # Check if there are security validation steps
                        ...
                        if not has_security_steps:
                            issues.append({
                                "issue_type": "missing_security_validation",
                                "severity": "medium",
                                "description": f"Security-sensitive function at index {function_idx} in file '{file_path}' lacks security validation steps",
                                "file_path": file_path,
                                "function_idx": function_idx
                            })
...
def _validate_implementation_test_alignment(
    ...
    """
    Validate alignment between implementation plan and test requirements.
    ...
    Returns:
        List of test alignment validation issues
    """
    issues = []
    
    # Get all implemented elements
    ...
    for file_path, functions in implementation_plan.items():
        if isinstance(functions, list):
            for function_idx, function in enumerate(functions):
                if isinstance(function, dict) and "element_id" in function:
                    element_id = function["element_id"]
                    implemented_elements[element_id] = (file_path, function_idx, function)
    
    # Check if all tested elements are implemented
    for element_id, tests in test_requirements.items():
        if element_id not in implemented_elements:
            issues.append({
                "issue_type": "unimplemented_tested_element",
                "severity": "critical",
                "description": f"Element '{element_id}' has tests but is not implemented",
                "element_id": element_id
            })
            continue
        
        # Check if implementation covers test requirements
        file_path, function_idx, function = implemented_elements[element_id]
        
        # Combine test requirements for this element
        test_behaviors = set()
        test_edge_cases = set()
        
        for test in tests:
            # Extract test behaviors from test description
            if isinstance(test, dict):
                test_description = test.get("description", "").lower()
                
                # Extract key behaviors
                for sentence in test_description.split("."):
                    sentence = sentence.strip()
                    if "should" in sentence or "must" in sentence or "verify" in sentence or "check" in sentence:
                        test_behaviors.add(sentence)
                
                # Extract test code to identify edge cases
                test_code = test.get("code", "").lower()
                
                # Look for edge cases in the test code
                if "none" in test_code or "null" in test_code:
                    test_edge_cases.add("Handle None/null input")
                if "empty" in test_code or "''" in test_code or '""' in test_code:
                    test_edge_cases.add("Handle empty string")
                if "invalid" in test_code or "exception" in test_code or "error" in test_code:
                    test_edge_cases.add("Handle invalid input")
        
        # Check if implementation edge cases cover test edge cases
        implementation_edge_cases = set(function.get("edge_cases", []))
        missing_edge_cases = [ec for ec in test_edge_cases if not any(ec.lower() in iec.lower() for iec in implementation_edge_cases)]
        
        if missing_edge_cases:
            issues.append({
                "issue_type": "missing_edge_cases",
                "severity": "medium",
                "description": f"Implementation for element '{element_id}' is missing edge cases covered in tests: {', '.join(missing_edge_cases)}",
                "file_path": file_path,
                "function_idx": function_idx,
                "element_id": element_id,
        ...
        # Check if implementation steps cover test behaviors
        steps = function.get("steps", [])
        step_descriptions = []
        
        for step in steps:
            if isinstance(step, dict) and "step_description" in step:
                step_descriptions.append(step["step_description"].lower())
        
        # Combine step descriptions into a single string for easier matching
        steps_text = " ".join(step_descriptions)
        
        # Check if each test behavior is covered in implementation steps
        missing_behaviors = []
        
        for behavior in test_behaviors:
            # Extract key verbs and nouns from the behavior
            words = set(behavior.split())
            
            # Check if implementation steps cover the behavior
            covered = False
            for word in words:
                if len(word) > 4 and word in steps_text:  # Only consider significant words
                    covered = True
                    break
        ...
        if missing_behaviors:
            issues.append({
                "issue_type": "missing_test_behaviors",
                "severity": "medium",
                "description": f"Implementation for element '{element_id}' may not cover test behaviors: {', '.join(missing_behaviors[:2])}{'...' if len(missing_behaviors) > 2 else ''}",
                "file_path": file_path,
                "function_idx": function_idx,
                "element_id": element_id,
