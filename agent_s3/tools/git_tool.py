...
class GitTool:
    ...
    def run_git_command(self, command: str, max_retries: int = None, retry_delay: int = None) -> Tuple[int, str]:
        ...
        while retries <= max_retries:
            try:
                ...
                if error_category == 'network' and retries < max_retries:
                    self.logger.warning(f"Network error detected in Git command. Retrying in {current_delay}s: {output}")
                ...
                if retries < max_retries:
                    self.logger.warning(f"Retrying Git command in {current_delay}s after error")
                    time.sleep(current_delay)
                    retries += 1
                    current_delay *= 2  # Exponential backoff
                else:
            ...
            try:
                ...
                if response.status_code == 404:
                    ...
                    if "rate limit" in response.text.lower():
                        ...
                        if retries < max_retries:
                            self.logger.warning(f"Rate limit hit. Waiting {wait_time:.1f}s and retrying...")
                    ...
                    if retries < max_retries:
                        self.logger.warning(f"GitHub server error {response.status_code}. Retrying in {retry_delay}s...")
                    ...
                    if retries < max_retries and response.status_code in (408, 429, 502, 503, 504):
                        self.logger.warning(f"GitHub API error {response.status_code}. Retrying in {retry_delay}s...")
                        time.sleep(retry_delay)
                        retries += 1
                        retry_delay *= 2  # Exponential backoff
                        continue
                ...
                if retries < max_retries:
                    self.logger.warning(f"GitHub API request timed out. Retrying in {retry_delay}s...")
                    time.sleep(retry_delay)
                    retries += 1
                    retry_delay *= 2  # Exponential backoff
                    continue
                ...
                if retries < max_retries:
                    self.logger.warning(f"GitHub API connection error. Retrying in {retry_delay}s...")
                    time.sleep(retry_delay)
                    retries += 1
                    retry_delay *= 2  # Exponential backoff
                    continue
                ...
                if retries < max_retries:
                    self.logger.warning(f"Unexpected error. Retrying in {retry_delay}s...")
                    time.sleep(retry_delay)
                    retries += 1
                    retry_delay *= 2  # Exponential backoff
                    continue
    ...
    def create_branch(self, branch_name: str, source_branch: Optional[str] = None) -> Dict[str, Any]:
        ...
        if branch_exists:
            self.logger.warning(f"Branch {branch_name} already exists. Attempting to check it out.")
        ...
        if source_branch:
            ...
            if code != 0:
                ...
                self.logger.warning(f"Source branch {source_branch} not found locally. Attempting to fetch.")
        ...
        if code != 0:
            ...
            if error_category == 'network':
                self.logger.warning(f"Network issue during pull. Continuing without latest changes: {output}")
    ...
    def branch_exists(self, branch_name: str) -> Tuple[bool, str]:
        ...
        if code != 0:
            self.logger.warning(f"Failed to fetch branches: {output}")
    ...
    def get_repository_info(self) -> Dict[str, Any]:
        ...
        if code != 0:
            self.logger.warning(f"Failed to get remote URL: {output}")
            return result
        ...
        self.logger.warning(f"Failed to parse owner and repo from remote URL: {remote_url}")
        return result
    ...
    def _get_current_branch(self) -> Optional[str]:
        ...
        code, output = self.run_git_command("branch --show-current")
        if code != 0:
            self.logger.warning(f"Failed to get current branch: {output}")
            return None
    ...
    def create_pull_request(self, title: str, body: str, head_branch: Optional[str] = None, 
        ...
        if success and isinstance(response, dict):
            ...
            error_msg = response if isinstance(response, str) else "Unknown error"
            self.logger.error(f"Failed to create pull request: {error_msg}")
            return None
    ...
    def _check_rate_limit(self) -> bool:
        ...
        if self.rate_limit_remaining <= 0 and time.time() < self.rate_limit_reset:
            ...
            if wait_time > 0:
                self.logger.warning(f"GitHub API rate limit exceeded. Reset in {wait_time:.1f} seconds.")
                return False
