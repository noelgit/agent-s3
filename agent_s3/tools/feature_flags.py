        ...
        with self._lock:
            try:
                ...
                if not config_file.exists():
                    logger.info(f"Feature flag configuration not found at {self.config_path}. Creating default.")
                    self._features = {
                        "features": {},
                ...
                logger.debug(f"Loaded {len(self._features.get('features', {}))} feature flags")
                return True
            except Exception as e:
                logger.error(f"Error loading feature flag configuration: {e}")
    ...
    def _save_configuration(self) -> bool:
        ...
        with self._lock:
            try:
                ...
                self._last_modified = os.path.getmtime(self.config_path)
                logger.debug(f"Saved feature flag configuration with {len(self._features.get('features', {}))} flags")
                return True
            except Exception as e:
                logger.error(f"Error saving feature flag configuration: {e}")
                return False
    ...
    def _watch_config_file(self) -> None:
        ...
        while not self._stop_watching.is_set():
            try:
                ...
                logger.error(f"Error checking feature flag configuration file: {e}")
