...
class DependencyAnalyzer:
    ...
    def extract_dependencies(self, file_path: str, language: str = None):
        ...
        if parser:
            try:
                logger.info(f"Extracting dependencies from {file_path} with {type(parser).__name__} for language '{language}'.")
    ...
    def get_dependency_versions(self, packages: List[str]) -> Dict[str, str]:
        ...
        for pkg in packages:
            try:
                ...
                logger.error(f"Error getting version for {pkg}: {e}")
        
        return versions
    
    def analyze_code_changes(self, changes: List[Dict], language_map: Optional[Dict[str, str]] = None) -> Dict:
        ...
        for change in changes:
            ...
            if not language:
                logger.debug(f"Skipping dependency analysis for unsupported file type: {file_path}")
                continue
