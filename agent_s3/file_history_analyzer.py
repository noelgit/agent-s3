...
class FileHistoryAnalyzer:
    ...
    def get_file_modification_info(self) -> Dict[str, Dict[str, Any]]:
        ...
        try:
            ...
            if not self.git_tool:
                self._log("GitTool not available, unable to analyze file history", level="warning")
                return {}
            ...
            for file_path, count in file_counts.items():
                file_info[file_path] = {
                    ...
                    'last_modified': current_time - timedelta(days=file_last_modified.get(file_path, 0))
                }
    ...
    def get_recently_modified_files(self, days: int = 7, max_files: int = 10) -> List[str]:
        ...
        recent_files = [
            path for path, info in file_info.items() 
