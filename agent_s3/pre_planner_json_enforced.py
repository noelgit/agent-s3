...
def pre_planning_workflow(
    ...
    for round_num in range(max_rounds):
        ...
        if status == "success":
            ...
            # --- Write the pre-plan JSON for human review ---
            try:
                with open(preplan_path, "w", encoding="utf-8") as f:
                    json.dump(data, f, indent=2, ensure_ascii=False)
                logger.info(f"Wrote preplan file for human review at {preplan_path}")
            except Exception as e:
                logger.error(f"Failed to write preplan file {preplan_path}: {e}")
                print(f"\n\033[91mError: Could not write preplan file {preplan_path}: {e}\033[0m")
                return False, {}
            ...
            try:
                ...
                logger.error(f"Failed to read preplan file {preplan_path}: {e}")
                print(f"\n\033[91mError: Could not read preplan file {preplan_path}: {e}\033[0m")
                return False, {}
            valid, all_errors = validate_preplan_all(preplan_data)
            if not valid:
                logger.warning(
                    f"Human-edited preplan file {preplan_path} is invalid: {all_errors}"
                )
                print(
                    f"\n\033[91mError: {preplan_path} is invalid after human edit. Please fix the file and rerun.\033[0m"
                )
                return False, {}
