...
class DeploymentManager:
    ...
    def execute_deployment(self, deployment_config: Optional[DeploymentConfig] = None) -> Dict[str, Any]:
    ...
    def _generate_launch_command(self, config: DeploymentConfig) -> str:
        ...
        if config.app_type == "flask":
            cmd += f"{venv_prefix + ' && ' if venv_prefix else ''}flask --app {config.entry_point.rstrip('.py')} run --host={config.host} --port={config.port} {'--debug' if config.debug_mode else ''}"
        elif config.app_type == "django":
            cmd += f"{venv_prefix + ' && ' if venv_prefix else ''}python manage.py runserver {config.host}:{config.port}"
        elif config.app_type == "fastapi":
            cmd += f"{venv_prefix + ' && ' if venv_prefix else ''}uvicorn {config.entry_point.rstrip('.py')}:app --host {config.host} --port {config.port} {'--reload' if config.debug_mode else ''}"
...
class CustomHandler(SimpleHTTPRequestHandler):
    def do_GET(self):
        if self.path == '/api/status':
            ...
            self.end_headers()
            self.wfile.write(bytes('{{"status": "ok", "app_name": "' + os.getenv('APP_NAME', 'Python App') + '"}}', 'utf-8'))
            return
        return super().do_GET()
...
"""
    ...
    def _save_deployment_log(self, config: DeploymentConfig, status: str, error: Optional[str] = None) -> None:
    ...
    def _extract_deployment_config(self, response: str) -> Optional[DeploymentConfig]:
        ...
        try:
            ...
            app_type_match = re.search(r"Application Type:?\s*([a-zA-Z0-9_+\- ]+)", response, re.IGNORECASE)
            app_type = app_type_match.group(1).strip().lower() if app_type_match else "python"
            ...
            app_name_match = re.search(r"DEPLOYMENT SUMMARY.*?([a-zA-Z0-9_\- ]+)(?:application|app|service)", response, re.IGNORECASE | re.DOTALL)
            app_name = app_name_match.group(1).strip() if app_name_match else f"{app_type.title()} Application"
            
            # Extract debug/environment mode
            debug_match = re.search(r"(?:Environment Mode|Mode):?\s*([a-zA-Z0-9_\- ]+)", response, re.IGNORECASE)
            debug_mode = debug_match and "dev" in debug_match.group(1).lower() or "development" in debug_match.group(1).lower()
