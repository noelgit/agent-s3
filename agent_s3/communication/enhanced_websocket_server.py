...
class EnhancedWebSocketServer:
    ...
    def _handle_interactive_approval(self, message: Message):
        ...
        logger.info(
            "Interactive approval request %r with %d options (ID: %s)",
            title,
            len(options),
            request_id,
        )
    ...
    def _handle_progress_indicator(self, message: Message):
        ...
        if steps:
            logger.info(
                "Progress indicator %r at %d%% (%s steps completed)",
                title,
                percentage,
                completion_ratio,
            )
    ...
    def _handle_thinking(self, message: Message):
        ...
        logger.debug(
            "Agent thinking indicator received from %s (stream: %s)",
            source,
            stream_id,
        )
    ...
    def _handle_stream_start(self, message: Message):
        ...
        logger.debug(
            "Content stream started from %s (stream: %s)",
            source,
            stream_id,
        )
    ...
    def _handle_stream_content(self, message: Message):
        ...
        if stream_id not in self._active_streams:
            logger.warning(
                "Received content for unknown stream: %s",
                stream_id,
            )
            self._active_streams.add(stream_id)
                ...
                try:
                    ...
                    if message_type == "authenticate":
                        ...
                        if token == self.auth_token:
                            ...
                            logger.info(
                                "Client %s authenticated successfully",
                                client_id,
                            )
                            ...
                            if client_preferences:
                                logger.info(
                                    "Setting preferences for client %s: %s",
                                    client_id,
                                    client_preferences,
                                )
                        else:
                    ...
                    if message_type == "reconnect":
                        ...
                        if previous_id and previous_id in self.authenticated_clients:
                            ...
                            if state_data:
                                logger.info(
                                    "Received state data from reconnecting client %s",
                                    client_id,
                                )
                    ...
                    logger.error("Invalid JSON from client %s", client_id)
                    await self.send_message(client_id, Message(
                        type=MessageType.ERROR_NOTIFICATION,
                    ...
                    logger.error(
                        "Error handling message from client %s: %s",
                        client_id,
                        e,
                    )
                    await self.send_message(client_id, Message(
    ...
    async def _handle_preferences_update(self, client_id: str, message_dict: Dict[str, Any]):
        ...
        logger.info(
            "Updated preferences for client %s: %s",
            client_id,
            preferences,
        )
    ...
    async def _send_batch(self, client_id: str) -> bool:
        ...
        try:
            ...
            logger.error(
                "Error sending batch to client %s: %s",
                client_id,
                e,
            )
    ...
    async def _send_queued_messages(self, old_id: str, new_id: str) -> int:
        ...
        logger.info(
            "Sent %d queued messages to client %s",
            count,
            new_id,
        )
        return count
            ...
            try:
                ...
                for client_id in expired_clients:
                    if client_id in self.client_queues:
                        ...
                        logger.info(
                            "Expired queue for client %s, dropped %d messages",
                            client_id,
                            queue_size,
                        )
                ...
                logger.error("Error in queue cleaner: %s", e)
    ...
    async def _send_heartbeats(self):
        ...
        while self.running:
            try:
                ...
                logger.error("Error sending heartbeats: %s", e)
                await asyncio.sleep(self.heartbeat_interval)
    ...
    async def _log_metrics(self):
        ...
        while self.running:
            try:
                ...
                # Log metrics
                logger.info("WebSocket Server Metrics: %s", all_metrics)

                await asyncio.sleep(60)  # Log every minute
            except asyncio.CancelledError:
                break
            except Exception as e:
                logger.error("Error logging metrics: %s", e)
                await asyncio.sleep(60)
    ...
    async def start(self):
        ...
        logger.info(
            "EnhancedWebSocketServer running on %s:%s",
            self.host,
            self.port,
        )
        ...
        if self.running:
            ...
            if os.path.exists(self.connection_file):
                try:
                    ...
                    logger.error("Error removing connection file: %s", e)
    ...
    def start_in_thread(self):
        ...
        def run_server():
            ...
            try:
                ...
                logger.error("Error in WebSocket server thread: %s", e)
            finally:
