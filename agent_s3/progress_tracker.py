...
class ProgressTracker:
    ...
    def update_progress(self, entry_data: Dict[str, Any]) -> None:
        ...
        # Broadcast via WebSocket if available and loop is set
        if self.websocket_server and self.websocket_server.running and self.loop:
            try:
                # Use model_dump to get a dict suitable for JSON serialization
                entry_dict = entry.model_dump(mode='json')
                # Ensure broadcast_progress_update is a coroutine
                coro = self.websocket_server.broadcast_progress_update(entry_dict)
                # Schedule the coroutine safely from any thread
                self.loop.call_soon_threadsafe(self.loop.create_task, coro)
            except Exception as e:
                 self.logger.error(f"Failed to broadcast progress update via WebSocket: {e}")
        elif self.websocket_server and not self.loop:
             self.logger.warning("WebSocket server is set but no event loop is available in ProgressTracker for broadcasting.")
