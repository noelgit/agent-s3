...
class ProgressTracker:
    ...
    def log_entry(self, entry: ProgressEntry) -> bool:
        ...
        try:
            ...
            # Convert back to Status enum for logging if enum values are stored
            status_val = validated_entry.status
            status_enum = (
                Status(status_val) if not isinstance(status_val, Status) else status_val
            )

            # Also log to standard logger
            self.logger.info(
                f"Progress: {validated_entry.phase} - {status_enum.name} - {validated_entry.details or ''}"
            )
