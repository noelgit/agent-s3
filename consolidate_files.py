...
def consolidate_duplicate_files():
    ...
    for duplicate, original in duplicates:
        ...
        if duplicate_content is None and original_content is None:
            logger.warning(f"Both files don't exist: {duplicate} and {original}")
            continue
        ...
        if not backup_file(original):
            logger.error(f"Failed to backup original file: {original}. Skipping merge.")
            continue
        ...
        if write_file(original, merged_content):
            ...
            logger.error(f"Failed to write merged content to {original}")
