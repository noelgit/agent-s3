...
class MockLLMResponses:
    ...
    def _create_feature_addition_plan(task: str, file_paths: List[str] = None) -> Dict[str, Any]:
        ...
        feature_match = re.search(r'(?:add|create|implement)\s+(?:a|an)?\s*([a-zA-Z_]+(?:\s+[a-zA-Z_]+)?)', task.lower())
        feature_name = feature_match.group(1) if feature_match else "new_feature"
        ...
        plan = {
            ...
            "plan": [
                ...
                f"Add unit tests in {file_paths[1] if len(file_paths) > 1 else 'tests/test_' + implementation_files[0].split('/')[-1]}",
    ...
    def _create_bug_fix_plan(task: str, file_paths: List[str] = None) -> Dict[str, Any]:
        ...
        plan = {
            ...
            "plan": [
                ...
                f"Add regression tests in {file_paths[1] if len(file_paths) > 1 else 'tests/test_' + implementation_files[0].split('/')[-1]}",
                "Verify tests pass and the bug is fixed"
            ],
            "test_specifications": []
        }
    ...
    def _create_refactoring_plan(task: str, file_paths: List[str] = None) -> Dict[str, Any]:
        ...
        refactor_match = re.search(r'(?:refactor|improve|optimize)\s+([a-zA-Z_]+(?:\s+[a-zA-Z_]+)?)', task.lower())
        refactor_area = refactor_match.group(1) if refactor_match else "component"
        ...
        plan = {
            ...
            "plan": [
                ...
                f"Add additional tests in {file_paths[1] if len(file_paths) > 1 else 'tests/test_' + implementation_files[0].split('/')[-1]} to cover edge cases",
                "Verify tests pass and functionality is preserved"
            ],
            "test_specifications": []
        }
    ...
    def _generate_implementation_code(file_path: str, function_names: List[str], task: str) -> str:
        ...
        if is_python:
            ...
            for function_name in function_names:
                ...
                code += f"        param1 = param1.replace('<script>', '').replace('</script>', '')\n\n"
                code += f"    # Process based on params\n"
                code += f"    if not param2:\n"
