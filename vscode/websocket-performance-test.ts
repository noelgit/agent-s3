...
export class WebSocketPerformanceTest {
  ...
  constructor() {
    ...
    this.outputChannel = vscode.window.createOutputChannel("WebSocket Performance Test");
  }
  ...
  private generateTestData(size: number): string {
    ...
    while (result.length < size) {
      result += pattern.substring(0, Math.min(pattern.length, size - result.length));
    }
 ...
 */
export function registerPerformanceTestCommand(context: vscode.ExtensionContext): void {
  const performanceTest = new WebSocketPerformanceTest();
  
  const command = vscode.commands.registerCommand('agent-s3.testWebSocketPerformance', () => {
