  ...
  public async testConnection(): Promise<void> {
    try {
      ...
      if (sent) {
        ...
        vscode.window.showWarningMessage("Test message queued for sending later");
      }
    } catch (error) {
      this.statusBarItem.text = "$(error) WS: Error";
      vscode.window.showErrorMessage(`Error testing WebSocket connection: ${error}`);
 ...
 */
export function initializeWebSocketTester(context: vscode.ExtensionContext): WebSocketTester {
  const tester = new WebSocketTester();
  
  // Register command
  const command = vscode.commands.registerCommand('agent-s3.testWebSocket', () => {
